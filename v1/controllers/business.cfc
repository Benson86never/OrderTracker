component accessors="true" {
    ...
    public void function viewBusiness(rc){
      ...
      if(structKeyExists(form, 'EMAIL')) {
       ...
       if(isDefined("session.userResult.errorMsg")
        && session.userResult.errorMsg == ""){
          location("index.cfm?action=business.viewBusiness&userid=#encrypt(session.secure.personId, application.uEncryptKey, "BLOWFISH", "Hex")#", false);
       }
      }
    }
  }