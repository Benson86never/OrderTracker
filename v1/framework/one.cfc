component {
    ...
    */
    ...
    public string function uEncrypt(string str) {
        local.item = trim(str);
        try {
            return len(local.item) > 0
                ? "_" & encrypt(local.item, application.uEncryptKey, "AES", "Hex")
                : "";
        } catch(any e) {
            return str;
        }
    };
    public string function uDecrypt(string str, numeric strict = 0) {
        local.item = trim(str);
        try {
            if(len(local.item) > 0) {
                return left(local.item, 1) == "_"
                    ? decrypt(listLast(local.item, "_"), application.uEncryptKey, "AES", "Hex")
                    : "";
            } else {
                return "";
            }
        } catch(any e) {
            return arguments.strict ? '' : str;
        }
    };
}
